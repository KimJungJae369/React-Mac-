...
/*
    ...
    = 그런데 컴포넌트를 시작 태그와 종료 태그를 함께 사용하는 형태 '<컴포넌트>내용</컴포넌트>'로도 사용할 수 있다
    = 이러한 형태를 사용하면 컴포넌트 안에 포함된 내용을 특별한 'props'으로 취급한다 리액트에서는 이를 'children' 이라고 부른다
    = 리액트에서는 'Children'이 텍스트, JSX 요소, 숫자, 배열 등 다양한 형태가 될 수 있기 때문에 이를 유연하게 처리할 수 있는 'React Node' 타입으로 지정
    ...
    <ref와 key 속성은 props로 전달 불가능 -리액트 18까지- >
